...
var run = function run(element, config, options) {
  progress = progress.then(function () {
    ...
    ).then(report)["catch"](function (error) {
      return _addons["default"].getChannel().emit(_constants.EVENTS.ERROR, String(error));
    });
  });
};
  ...
  wrapper: function wrapper(getStory, context, _ref) {
    ...
    if (parameters) {
      if (storedDefaultSetup === null) {
        storedDefaultSetup = Object.assign({}, setup);
      }
